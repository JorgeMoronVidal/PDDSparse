...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    std::map<std::vector<int>, int> in_index, in_index2;
    std::vector<Eigen::VectorXf> Node_pos;
    interface_metadata(start, end, i_index, n_index, i_N, n_N, N_node, SW, NE, in_index);
    interface_metadata(Node_pos, n_index, i_index, in_index2, i_N, n_N, SW, NE);
    //c2 is defined
    float c2 = fac/N_node;
    ...
    //I am the server who distributes task
        ...
        while(work_control[1] == 0){
            ...
            if(work_control[1] == 0){
                ...
                sleep(2 + work_control[0]);
                std::cout << Node_pos[n_index[in_index[i_index[work_control[0]]]].back()] - end[in_index[i_index[work_control[0]]]] << std::endl << 
                          Node_pos[n_index[in_index[i_index[work_control[0]]]].front()] - start[in_index[i_index[work_control[0]]]] << std::endl;
...
void interface_metadata(std::vector<Eigen::VectorXf> & node_positions,
   ...
   for (int i = 0; i < SW.size() ; i++){
    ...
    fullfill_node_pos(node_positions, inter_indexes, node_indexes, interface_map,start, end);
...
void compute_ipsi(std::vector<std::vector<float> > & ipsi_val,
    ...
    #ifdef DEBUG
        std::cout << "Inverse matrix computed with error "<< err <<" and condition number" << std::endl;
    #endif
    Psi.resize(0,0);
